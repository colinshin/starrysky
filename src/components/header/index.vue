            ...
            <div id="navigation" class="collapse navbar-collapse flex-column" >
                ...
                <ul class="navbar-nav flex-column text-left">
                    ...
                    <template v-for="(value,key,index) in catelist">
                        <li v-if="current_nav_id == value.id" class="nav-item active"  :v-bind="key">
                            <a class="nav-link" href="javascript:void(0);" @click="toPage('/', value.id)"><i class="fas fa-home fa-fw mr-2"></i>{{value.c_name}}</a>
                        </li>
                        <li v-else class="nav-item" >
                            <a class="nav-link" href="javascript:void(0);" @click="toPage('/', value.id)"><i class="fas fa-home fa-fw mr-2"></i>{{value.c_name}}</a>
                        </li>
    ...
    export default {
        ...
        data() {
            return {
                ...
                catelist:[],
                toPage:function(page_router,category_id){
                    console.log(category_id)
                    this.current_nav_id = category_id
                    this.$router.push({
                        path: page_router,
                        query: {
