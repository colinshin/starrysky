                ...
                <div v-for="(value,key,index) in datalist" class="item mb-5">
                    <div class="media">
                        ...
                        <div class="media-body">
                            <h3 class="title mb-1"><a href="javascript:void(0);" @click="toArticle(value.id)">{{value.title}}</a>
                            </h3>
                            ...
                            <div class="intro">{{value.summary}}...</div>
                            <a class="more-link" href="javascript:void(0);" @click="toArticle(value.id)">Read
                                more &rarr;</a>
                        </div><!--//media-body-->
                    </div><!--//media-->
                </div><!--//item-->
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                category_id:this.$route.query.category_id?this.$route.query.category_id:1,
                page: 1,
                page_size: 10,
                msg: 'this is index',
                datalist: [],
                ...
                getMore: function () {
                    this.getList(this.page, this.category_id)
                }
            }
        },
        components: {},
        ...
        watch:{
            $route(to,from){
                if(to.query.category_id != this.category_id) {
        ...
        methods: {
            getList: function () {
                axios.get(this.Api.getList + "?page=" + this.page + "&page_size=" + this.page_size+ "&category_id=" + this.category_id).then(res => {
                    if (res.data.ret == 0) {
                        if(res.data.data.length > 0) {
                            res.data.data.forEach((item, index, arr) => {
                                if (item.img_path == '') {
                                    arr[index].img_path = this.getDefaultImg(item.id)
                                }
                            })
